            ...
            <form action="{{ url('/admin/settings/sections/'.$id.'/links/'.$link->id) }}" method="post" id="submit">
                ...
                <div class="row">
                    <div class="col-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="links_published_at" class="form-control"
                                   value="@if(old('links_published_at')){{ old('links_published_at') }}@else{{ date('Y-m-d', strtotime($link->published_at)) }}@endif"
                                   id="datepicker">
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="links_published_time" class="form-control timepicker"
                                   value="@if(old('links_published_time')){{ old('links_published_time') }}@else{{ date('H:i', strtotime($link->published_at)) }}@endif">
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="form-group">
                                ...
                                <select class="form-control" name="links_rubric_id">
                                    ...
                                    @if (!is_null($rubrics))
                                        @foreach ($rubrics as $rubric)
                                            <option
                                                value="{{ $rubric->id }}" @if(old('links_rubric_id') == $rubric->id){{ 'selected' }}@elseif($link->rubric_id == $rubric->id){{ 'selected' }}@endif>{{ !is_null($rubric->title_ru) ? $rubric->title_ru : str_limit(strip_tags($rubric->description_ru), 100) }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                ...
                <ul class="nav nav-tabs" role="tablist">
                    @foreach($langs as $lang)
                        <li class="nav-item">
                            <a id="tabClick" class="nav-link @if($lang->key == 'ru') active show @endif" href="#title_{{ $lang->key }}" data-lang="{{$lang->key}}"
                               data-toggle="tab">
                                {{ $lang->name }}
                            </a>
                        </li>
                ...
                <div class="tab-content">
                    @foreach ($langs as $lang)
                        <div class="tab-pane @if($lang->key == "ru") active show @endif" id="title_{{ $lang->key }}" role="tabpanel">
                            ...
                            <div class="row">
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="links_title_{{$lang->key}}"
                                               value="@if(old('links_title_' . $lang->key )){{ old('links_title_' . $lang->key ) }}@else{{ $link->$title }}@endif">
                                    </div>
                                </div>
                                ...
                                <div class="col-5">
                                    <div class="form-group">
                                        <label>Ссылка</label>
                                        <input type="text" class="form-control" name="links_link_{{$lang->key}}"
                                               value="@if(old('links_link_' . $lang->key )){{ old('links_link_' . $lang->key ) }}@else{{ $link->$link_lang }}@endif">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-9">
                                    <textarea class="form-control tinymce" name="links_description_{{$lang->key}}"
                                              rows="15">@if(old('links_description_' . $lang->key )){{ old('links_description_' . $lang->key ) }}@else{{ $link->$description }}@endif</textarea>
                                </div>
                                <div class="col-3">
                                    <div class="form-group">
                                                ...
                                                @if (!$link->$photo == '')
                                                    <div class="card card-hover">
                                                        <div class="card-header text-right">
                                                            <a href="#" class="deletePhoto" data-model="Links" data-id="{{ $link->id }}" data-lang="{{ $lang->key }}"><i
